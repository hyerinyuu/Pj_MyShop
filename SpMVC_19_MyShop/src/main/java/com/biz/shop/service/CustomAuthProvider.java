...
public class CustomAuthProvider implements AuthenticationProvider{
	...
	public Authentication authenticate(Authentication authentication) throws AuthenticationException {
		...
		String username = (String)authentication.getPrincipal(); // username추출
		...
		/*
		 * 만약 해당 사용자의 detail한 정보가 DB에 있다면
		 * 해당 정보를 DB에서 조회하여 
		 * customUserDetails Class에 세팅하여 Controller로 전송할 수 있다.
		 * 
		 */
		...
		UsernamePasswordAuthenticationToken token = new UsernamePasswordAuthenticationToken(username, password, roles); //principal, credentials, authorities
		
		token.setDetails(cUserDetails);
		// login_ok controller로 전송됨
		return token;
	}
	...
	public boolean supports(Class<?> authentication) {
		...
		// 사용자 정보에 token이 들어있는지 확인해서 들어있으면 true를 없으면 false를 loginController로 return
