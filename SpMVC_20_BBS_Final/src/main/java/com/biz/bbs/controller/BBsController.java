...
public class BBsController {
	 ...
	 * BBS_LIST는 이름으로 model에 Attribute를 추가하여
	...
	/*
	 * 리스트에서 작성 버튼을 클릭했을 때 게시판 작성 화면을 열어줄 path
	 * 버튼을 클릭했을 때 작동할 path는 method를 GET로 설정한다.
	 */
	...
	/*
	 * 게시판 작성화면에서 저장 버튼을 클릭했을 때
	 * 게시판 작성화면은 <form> tag로 감싸져 있고
	 * form tag내에 input tag로 입력상자를 구성하고 버튼은 2개 있음
	 * 
	 * 그중 저장버튼은 아무 type도 지정하지 않았기 때문에
	 * 저장버튼을 클릭하면 <form> tag에 지정된 
	 * action path로 method에 지정된 방식으로 데이터를 전송한다.
	 * - 이러한 버튼의 기본 기능을 submit이라고 한다.
	 * 
	 * bbs_write.jsp에서는 <form> tag에 action을 제거했다.
	 * -이렇게 되면 처음 게시판 작성 화면을 열기 위해서 사용했던 path인
	 * - ${rootPath}/bbs/insert가 action에 자동으로 설정이 된다.
	...
	public String insert(BBsVO bbsVO) {
		...
		// action을 임의로 지정 했을때
		// form에 action tag로 insert를 넣어줘서
		// update한 상황에서 새로 insert가 수행되는 상황을 막기 위한 코드
	...
	/*
	 * 게시판 리스트에서 제목을 클릭하면
	 * update path에게 b_id값을 전달하고
	 * b_id값을 수신하여 게시판에서 1개의 게시판 데이터를 SELECT하고 bbsVO에 받는다.
	 * 
	 * model에 bbsVO를 BBS라는 이름으로 Attribute를 추가하고
	 * bbs_write.jsp파일에게 전달한다.
	 */
	...
	public String fileUp(MultipartFile upFile) {
		...
		log.debug("###업로드된 파일 원래 이름" + upFile.getOriginalFilename() );
		
		String returnFileName = fileService.fileUp(upFile);
