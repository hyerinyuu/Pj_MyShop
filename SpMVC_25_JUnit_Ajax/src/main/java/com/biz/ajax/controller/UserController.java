...
public class UserController {
	 ...
	 * vo에 한꺼번에 받기 위해서 사용하는 Annotation
	 * Spring3에서는 기본으로 설정을 해주어야만 제대로 vo에서 데이터를 받았다.
	 * 
	 * Spring4에서부터는 선택사항으로 설정되고 vo에 annotaion을 붙이지 않아도
	 * form에서 전송되는 데이터를 잘 받았다.
	 * 
	 * form의 input box에 name으로 설정된 이름의 변수가 
	 ...
	 * form에서 받은 데이터를 다시 view로 재전송할 때 Model에 데이터를 싣지 않아도 자동으로 값이 실려서 전송되었다.
	 * 
	 * @ModelAttribute("객체이름")을 설정해주어야 Model에 싣는 기능이 정상작동된다.
	 * 
	 * @ModelAttribute("객체이름")을 설정하면 form에서 vo에 받은 데이터를 별도로 Model에 싣지 않아도
	 * 바로 view로 전송할 수 있다.
	 */
	 ...
	 * return문에서 view파일을 찾지 않고
	 * return문의 데이터를 있는 그대로 전송하는 역할을 수행한다.
	 */
	@ResponseBody
